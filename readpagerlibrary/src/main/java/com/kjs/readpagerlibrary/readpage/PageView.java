...
import com.kjs.readpagerlibrary.readpage.animation.CoverPageAnim;
import com.kjs.readpagerlibrary.readpage.animation.HorizonPageAnim;
import com.kjs.readpagerlibrary.readpage.animation.NonePageAnim;
import com.kjs.readpagerlibrary.readpage.animation.PageAnimation;
import com.kjs.readpagerlibrary.readpage.animation.ScrollPageAnim;
import com.kjs.readpagerlibrary.readpage.animation.SimulationPageAnim;
...
public class PageView extends View {
    ...
    //滚动效果
    public final static int PAGE_MODE_SCROLL =2;
    //无效果
        ...
        switch (pageMode){
            ...
            case PAGE_MODE_SCROLL:
                mPageAnim = new ScrollPageAnim(mViewWidth, mViewHeight, 0,
                        DisplayUtil.dip2px(context,PageLoader.DEFAULT_MARGIN_HEIGHT),this,mPageAnimListener,mPageLoader);
                break;
            default:
                mPageAnim = new SimulationPageAnim(mViewWidth, mViewHeight,this,mPageAnimListener);
        }
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchListener != null) {
                    if(!mTouchListener.prohibitedSlide()){
                        //如果滑动了，则进行翻页。
                        if(isMove){
                            mPageAnim.onTouchEvent(event);
                        }
    ...
    //获取PageLoader
    public PageLoader getPageLoader(boolean isLocal, LoadingContentView loadingContentView, ReadActionListener readActionListener){
        if (mPageLoader == null){
            if (isLocal){
				//读取本地文件暂未实现
//              mPageLoader = new LocalPageLoader(this);
                ...
                mPageLoader = new NetPageLoader(context,this,loadingContentView,readActionListener);
            }
        }
        return mPageLoader;
    }
