package com.kjs.test.coffeelibrary.base.manager;
            ...
            for (int i = 0; i < deniedPermissions.size(); i++) {
                ...
                if (isLackPermission(deniedPermissions.get(i))) {
                    if (isTip) {//表明用户没有彻底禁止弹出权限请求
                        LogUtil.info("表明用户没有彻底禁止弹出权限请求弹窗"+deniedPermissions.get(i));
                        isRemind = true;
                    } else {//表明用户已经彻底禁止弹出权限请求
                        LogUtil.info("表明用户已经彻底禁止弹出权限请求弹窗"+deniedPermissions.get(i));
                        isNotRemind = true;
                    }
                }
            }
